...
public class PagesController {
	...
	public String Find(Model model, Produto produto, HttpServletRequest request, HttpServletResponse response) {
		...
		model.addAttribute("categorias", dao_cat.Read());
		model.addAttribute("produtos", dao_prod.Find_Many_publico(nome_prod, produto));
		return "Procurar";
	}
	...
	public String Find_Produto(Model model, @PathVariable("nome_produto") String nome_produto, Produto produto, Avaliacao avaliacao){
		model.addAttribute("produtos", dao_prod.Find_publico(nome_produto, produto));
		model.addAttribute("avaliacoes", dao_ava.Read(nome_produto, produto, avaliacao));
		model.addAttribute("categorias", dao_cat.Read());
	    return "Escolha";
	}
